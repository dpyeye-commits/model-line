...
import { TabGarments } from "./tab-garments";
...
export default async function CompanyDetailPage({
  ...
  const { tab = "fabrics", welcome } = await searchParams;
  ...
  // 탭별 데이터 로드
  const [fabricsRes, garmentsRes, filesRes, videosRes] = await Promise.all([
    supabase.from("fabrics").select("*").eq("brand_id", id).order("created_at", { ascending: false }),
    supabase.from("garment_models").select("*").eq("brand_id", id).order("created_at", { ascending: false }),
    supabase.from("company_files").select("*").eq("brand_id", id).order("created_at", { ascending: false }),
    supabase.from("promo_videos").select("*").eq("brand_id", id).order("created_at", { ascending: false }),
  ]);

  const fabricList = fabricsRes.data ?? [];
  const garmentList = garmentsRes.data ?? [];
  const fileList = filesRes.data ?? [];
  const videoList = videosRes.data ?? [];

  const totalDone = fabricList.reduce((s: number, f: any) => s + MAP_FIELDS.filter(m => f[m.key]).length, 0);
  ...
  return (
    <div className="p-8">
      {/* 웰컴 배너 (브랜드 첫 생성 시) */}
      {welcome === "1" && (
        <WelcomeBanner brandId={id} brandName={brand.name} />
      )}
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <BrandDescription brandId={id} description={brand.description} />
        </div>
        {tab === "fabrics" && (
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-zinc-400 text-sm">원단 디지털화 진행률</span>
          <span className="text-white font-bold">{pct}%</span>
        </div>
        <div className="w-full bg-zinc-800 rounded-full h-1.5">
          <div
            className={`rounded-full h-1.5 transition-all ${pct === 100 ? "bg-green-400" : pct > 0 ? "bg-amber-400" : "bg-zinc-700"}`}
            style={{ width: `${pct}%` }}
          />
        </div>
        <p className="text-zinc-600 text-xs mt-1.5">{totalDone} / {totalPossible} 항목 · 소재 {fabricList.length}종</p>
      </div>
      ...
      {tab === "garments" && (
        <TabGarments brandId={id} models={garmentList} />
      )}

      {tab === "library" && (
        <TabLibrary brandId={id} files={fileList} />
      )}

      {tab === "videos" && (
        <TabVideos brandId={id} videos={videoList} />
      )}
    </div>
  );
}
